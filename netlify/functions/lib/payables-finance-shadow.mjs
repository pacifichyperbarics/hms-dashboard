...
async function parity(config, expected) {
  const rows = await rest(config, 'hms_finance_payables', {
    query: 'select=external_key,amount_cents,status&source_type=eq.legacy_payables_blob',
  });
  const expectedByKey = new Map(expected.map((row) => [row.external_key, row]));
  ...
  for (const [key, row] of expectedByKey) {
    ...
    if (Number(actual.amount_cents) !== Number(row.amount_cents)) amountMismatch += 1;
    if (actual.status !== row.status) statusMismatch += 1;
  }
  ...
  return {
    ...
    amountMismatch,
    statusMismatch,
    matched: missing === 0 && extra === 0 && amountMismatch === 0 && statusMismatch === 0,
  };
}
...
export async function syncPayablesToFinance(state) {
  ...
  const parityStats = await parity(config, payableRows);
  ...
  await upsert(config, 'hms_sync_state', [{
    ...
    error_text: parityStats.matched ? null : 'Payables shadow parity mismatch',
    stats,
